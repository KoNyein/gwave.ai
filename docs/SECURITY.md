...
- **Amazon Cognito** (email/password + Google OAuth). စကားဝှက်များကို ကျွန်ုပ်တို့
  server မှ တိုက်ရိုက် မကိုင်ပါ — Cognito user pool က salted/hashed သိမ်းသည်။
  Cognito အောင်မြင်ပြီးမှ app က **ကိုယ်ပိုင် သက်တမ်းတို ES256 "data token"**
  (claims: `sub` = `profiles.id`, `role` = `authenticated`) ကို mint လုပ်ပြီး
  data API (self-hosted PostgREST + Realtime) က ကျွန်ုပ်တို့ JWKS နဲ့ စစ်သည်။
...
- Private **S3 bucket** (`slips`: KPay slip + KYC face) — public access ပိတ်ထား၍
  server မှ ထုတ်ပေးသော **သက်တမ်းတို signed URL** ဖြင့်သာ ဖတ်နိုင်
  (`src/lib/storage/signed-read.ts`); upload ကိုလည်း server က presign လုပ်ပေးပြီး
  object key ကို session user (`{uid}/…`) ကနေသာ တွက်သည်။

## 3. Transport & HTTP headers (`next.config.mjs`)

- **Content-Security-Policy** — `default-src 'self'`; script/style/img/connect
  များကို self + data API (`/sb`) + CloudFront/S3 media + allow-list ထားသော
  origin များသာ။ `object-src
...
- SQL — PostgREST client (parameterised; `@supabase/supabase-js` သည် ကျွန်ုပ်တို့
  self-hosted PostgREST အတွက် client ဖြစ်သည်) + `security definer` function များတွင်
...
- Service-role token ကို **server-only** — server က `APP_JWT_PRIVATE_KEY` ဖြင့်
  သက်တမ်းတို token ကို ကိုယ်တိုင် mint လုပ်သည် (client bundle ထဲ ဘယ်တော့မှ မထည့်);
  browser client သည် anon key + RLS သာ။
- `.env` များ commit မလုပ်; server `.env` (EC2 host) / AWS Secrets Manager သုံး။
...
- **2FA / TOTP** (Cognito MFA) — high-value account (G-Pay) များအတွက်။
