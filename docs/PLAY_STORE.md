...
- Production domain (ဥပမာ `gwave.cc`) — HTTPS ဖြင့် live ဖြစ်နေရမည်
...
bubblewrap init --manifest=https://gwave.cc/manifest.webmanifest
```
...
```bash
curl https://gwave.cc/.well-known/assetlinks.json
```
