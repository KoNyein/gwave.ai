...
```
  ...
  browsers / TWA  ───► │  ALB (HTTPS, ACM cert)    │
                       └────────────┬─────────────┘
                                    ▼
                       ┌──────────────────────────┐     ┌───────────────┐
                       │  ECS Fargate: gwave-web   │────►│  AWS backend  │
                       │  (Next.js standalone)     │     │  RDS+PostgREST│
                       └──────────────────────────┘     └───────────────┘
...
DB/Auth/Realtime/Storage အားလုံး **AWS** ပေါ်မှာ (2026-07-17 ကတည်းက Supabase
hosted service ကနေ ပြောင်းရွှေ့ပြီး) — DB က **RDS PostgreSQL**၊ data API က
**ကိုယ်ပိုင် self-hosted PostgREST + Realtime** (`https://gwave.cc/sb`)၊
Storage က **S3 + CloudFront**၊ Auth က **Amazon Cognito**။ Supabase project
မရှိတော့ပါ။ (`@supabase/supabase-js` package ကတော့ ဆက်သုံးတယ် — အဲဒါက
PostgREST client ဖြစ်လို့ AWS endpoint တွေနဲ့ တိုက်ရိုက် ကိုက်ညီပါတယ်။)
...
- RDS PostgreSQL (SQL migrations အားလုံး `psql` နဲ့ run ပြီး) + `/sb` data API
  (self-hosted PostgREST + Realtime)
...
docker build \
  --build-arg NEXT_PUBLIC_DATA_API_URL=https://gwave.cc/sb \
  --build-arg NEXT_PUBLIC_DATA_API_KEY=eyJ... \
...
```bash
for k in APP_JWT_PRIVATE_KEY APP_JWT_PUBLIC_JWK COGNITO_CLIENT_SECRET \
...
> `git pull` + command တစ်ခုနဲ့ တင်ချင်ရင် ဒါ အလွယ်ဆုံး။ Database က **RDS**
> (managed) မှာ ဖြစ်လို့ box က app ကိုပဲ run တယ်။ `deploy/docker-compose.prod.yml`
...
cp deploy/.env.server.example .env      # ← data API (/sb)/Stripe/LiveKit… ဖြည့်
...
| Var | Where | Feature |
| --- | --- | --- |
| `NEXT_PUBLIC_DATA_API_URL` / `NEXT_PUBLIC_DATA_API_KEY` | build-arg | core (self-hosted PostgREST + Realtime at `/sb`) |
| `APP_JWT_PRIVATE_KEY` / `APP_JWT_PUBLIC_JWK` | secret | server (mints the ES256 data token) |
| `COGNITO_*` (user pool, client id/secret, domain) | secret | auth |
...
> `NEXT_PUBLIC_DATA_API_URL/KEY` က `NEXT_PUBLIC_SUPABASE_URL` /
> `NEXT_PUBLIC_SUPABASE_ANON_KEY` ကနေ နာမည်ပြောင်းထားတာ — အဟောင်း ၂ ခုကိုလည်း
> `src/lib/env.ts` မှာ fallback အဖြစ် လက်ခံပါသေးတယ်။
...
- [ ] **Cognito** app client → Callback/Sign-out URLs မှာ
      `https://app.yourdomain.com/auth/callback` ထည့် (Supabase dashboard မဟုတ်တော့ပါ)
