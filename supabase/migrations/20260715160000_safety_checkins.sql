...
begin
  if not exists (
    ...
    where n.nspname = 'public' and t.typname = 'safety_status'
  ) then
...
create index if not exists safety_checkins_user_idx
  on public.safety_checkins (user_id, created_at desc);

alter table public.safety_checkins enable row level security;

drop policy if exists "Users check in as themselves" on public.safety_checkins;
...
-- both belong to.
drop policy if exists "Safety check-ins visible to self and family" on public.safety_checkins;
