...
export default async function AdminMembershipPage(
  ...
  // otherwise fall back to the legacy object storage (admin RLS grants read).
  // Without the S3 branch, uploads land in S3 while this still asks the legacy
  // backend for an object
  // that is no longer there — and the error is dropped, so the slip silently
  // renders as nothing.
  const slipUrls = new Map<string, string>();
  const db = s3SlipsEnabled() ? null : await createClient();
