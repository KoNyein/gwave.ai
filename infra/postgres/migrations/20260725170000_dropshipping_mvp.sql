-- Gwave dropshipping MVP for AWS RDS PostgreSQL.
-- Auth: Cognito data token -> self-hosted PostgREST -> auth.uid().
...
create trigger dropship_listings_set_updated_at before update on public.dropship_listings
for each row execute function public.handle_updated_at();
...
create trigger supplier_payouts_set_updated_at before update on public.supplier_payouts
for each row execute function public.handle_updated_at();

create index supplier_products_supplier_idx on public.supplier_products(supplier_id);
create index supplier_products_active_idx on public.supplier_products(is_active) where is_active;
...
create policy "supplier reads own profile" on public.supplier_profiles for select using (user_id = auth.uid());
create policy "supplier creates own profile" on public.supplier_profiles for insert with check (user_id = auth.uid());
create policy "supplier updates own profile" on public.supplier_profiles for update using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy "approved catalog is publicly readable" on public.supplier_products for select using (
  is_active and exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.status = 'approved')
);
create policy "supplier manages own products" on public.supplier_products for all using (
...
create policy "reseller manages own listings" on public.dropship_listings for all using (reseller_id = auth.uid()) with check (reseller_id = auth.uid());
create policy "active listings are publicly readable" on public.dropship_listings for select using (status = 'active');

create policy "supplier reads own fulfillment" on public.fulfillment_orders for select using (
  exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.user_id = auth.uid())
);
create policy "reseller reads own fulfillment" on public.fulfillment_orders for select using (reseller_id = auth.uid());
create policy "supplier updates own fulfillment" on public.fulfillment_orders for update using (
  exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.user_id = auth.uid())
);

create policy "participants read fulfillment items" on public.fulfillment_order_items for select using (
  exists (
    select 1 from public.fulfillment_orders fo
    left join public.supplier_profiles sp on sp.id = fo.supplier_id
    where fo.id = fulfillment_order_id and (fo.reseller_id = auth.uid() or sp.user_id = auth.uid())
  )
);

create policy "participants read shipments" on public.shipments for select using (
  exists (
    select 1 from public.fulfillment_orders fo
    left join public.supplier_profiles sp on sp.id = fo.supplier_id
    where fo.id = fulfillment_order_id and (fo.reseller_id = auth.uid() or sp.user_id = auth.uid())
...
create policy "supplier manages shipment" on public.shipments for all using (
  exists (
    select 1 from public.fulfillment_orders fo
...
create policy "supplier reads own ledger" on public.supplier_ledger_entries for select using (
  exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.user_id = auth.uid())
);
create policy "supplier reads own payouts" on public.supplier_payouts for select using (
  exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.user_id = auth.uid())
);
create policy "supplier requests own payout" on public.supplier_payouts for insert with check (
  exists (select 1 from public.supplier_profiles sp where sp.id = supplier_id and sp.user_id = auth.uid() and sp.status = 'approved')
);
